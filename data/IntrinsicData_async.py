...
class Image(object):
    def __init__(self, albedo, shading, image, mask):
        self.albedo = albedo
    ...
    def rot_crop_image(self, loc, imsize, angle):
        ...
        new_imsize = [int(math.ceil(ang * float(x))) for x in imsize]
    ...
    def scale_image_size(self, scale_val):
        im = Image(scipy.ndimage.zoom(self.albedo, scale_val + [1], order=1),
                scipy.ndimage.zoom(self.shading, scale_val + [1], order=1),
                scipy.ndimage.zoom(self.image, scale_val + [1], order=1),
                scipy.ndimage.zoom(self.mask, scale_val, order=0) > .5)
        if self.adversary_albedo is not None:
            im.adversary_albedo = scipy.ndimage.zoom(self.adversary_albedo, scale_val + [1], order=1)

        return im


    def rotate_image(self, angle):
        im = Image(scipy.ndimage.interpolation.rotate(self.albedo, angle, order=1, reshape=False),
                scipy.ndimage.interpolation.rotate(self.shading, angle, order=1, reshape=False),
                scipy.ndimage.interpolation.rotate(self.image, angle, order=1, reshape=False),
                scipy.ndimage.interpolation.rotate(self.mask, angle, order=0, reshape=False) > .5)
        if self.adversary_albedo is not None:
            im.adversary_albedo = scipy.ndimage.interpolation.rotate(self.adversary_albedo, angle, order=1, reshape=False)

        return im

    def scale_image(self, mult):
        im = Image(self.albedo,
                self.shading * mult,
                self.image * mult,
                self.mask)

        if self.adversary_albedo is not None:
            im.adversary_albedo = self.adversary_albedo

        return im

    def shift_towards_albedo(self, mult):
        im = Image(self.albedo,
                self.shading * mult + (1-mult),
                mult * self.image + (1-mult) * self.albedo,
                self.mask)
    ...
    def crop_image(self, loc, imsize):
        im = Image(Image.crop_im(self.albedo, loc, imsize),
                   Image.crop_im(self.shading, loc, imsize),
                   Image.crop_im(self.image, loc, imsize),
                   Image.crop_im(self.mask, loc, imsize))
        if self.adversary_albedo is not None:
            im.adversary_albedo = Image.crop_im(self.adversary_albedo, loc, imsize)
    ...
    def get_loc(self, imsize, sample_quality=.9):
        ...
        X = np.arange(h*w)
        X_img = X.reshape(h,w)
        while np.sum(self.crop_mask > sample_quality*np.prod(imsize)) < 10000:
            sample_quality*=.9
...
class IntrinsicData(object):
    def __init__(self, albedo_root, shading_root, image_root, mask_root,
            im_list = [], patch_size=[100,100], sample_quality=.95, color=False):

        if len(im_list) == 0:
            im_list = os.listdir(albedo_root)

        self.im_list = im_list

        self.albedo_root = albedo_root
    ...
    def get_image_obj(self, im_name):
        return Image(
            imread(self.albedo_root + im_name)/256.0,
            imread(self.shading_root + im_name)/256.0,
    ...
    def get_data(self, samples):
        ...
        batch['image'] = np.zeros(
                ...
                dtype=np.float32)
        batch['albedo'] = np.zeros(
                [batch_size] + self._patch_size + [3 if self._color else 1],
                dtype=np.float32)
        batch['shading'] = np.zeros(
                [batch_size] + self._patch_size + [3 if self._color else 1],
                dtype=np.float32)
        batch['mask'] = np.zeros(
        ...
        for i, single_sample in enumerate(batch_list):
            ...
            batch['shading'][i,:,:,:] = single_sample.shading
            batch['albedo'][i,:,:,:] = single_sample.albedo
...
def create_batch_generator(data_mgr, batch_size, queue_size, num_epochs, num_samples, same_process=False):
    ...
    index_generator = batch_creators.random(batch_size, num_samples, num_epochs)
...
def default_data(image_root, shading_root, albedo_root, mask_root, patch_size, batch_size, queue_size, num_samples, color=True, same_process=True, num_epochs=None):
    data_mgr = IntrinsicData(albedo_root, shading_root, image_root, mask_root, patch_size=patch_size, color=color)
